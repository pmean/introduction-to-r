...
`r readLines("../dat/airline-data-dictionary.txt")[3]`
...
You can also direct R output to a text file with the sink function. Read the help file on sink for more details.
...
```{r scatterplot-2}
plot(fd2$abdomen/2.54,fd2$fat_b,
...
trend.line <- lm(fat_b~abdomen.in,data=fd2)
plot(fd2$abdomen.in,fd2$fat_b)
abline(trend.line)
```
...
```{r trend-2}
smooth.curve <- lowess(fd2$age,fd2$fat_b)
plot(fd2$age,fd2$fat_b)
lines(smooth.curve)
```
