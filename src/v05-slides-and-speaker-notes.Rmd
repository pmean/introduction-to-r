...
```{r fev_means}
fev_means <- by(fev$fev, fev$smoke, mean, na.rm=TRUE)
fev_means
```
...
```{r fev_sd}
fev_stdev <- by(fev$fev, fev$smoke, sd, na.rm=TRUE)
fev_stdev
```
...
The data shown here seems to indicate the use of a single space as a delimiter.

</div>

```{r read.housing}
al <- read.table(
  file="../data/housing.txt",
  header=TRUE,
  sep=" ",
