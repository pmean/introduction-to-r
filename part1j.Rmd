...
To avoid the extra line at the top and the column names "1","2","3" use the options header=FALSE and col.names=TRUE. Try this and see what the two files look like now.
...
In other words, don't send a binary file to someone who does not have R. Binary files are useful when you are moving data from one R user to another, or data from one computer to another, or sometimes when you are moving dat from one folder to another.

We will not be using any of these files again, so there is no need to save anything.

There is no quize for part 1j.
